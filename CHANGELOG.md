...
## [v2.2.23](https://github.com/in2workspace/in2-issuer-api/releases/tag/v2.2.23)
## Fixed
- Removed strict `serialNumber` validation in `isMandatorIssuancePolicyValidLEARCredentialMachine` to allow the issuance of Machine Credentials without requiring this field.
...
### Changed
- Send label credential to response URI in parallel to sending the credential offer email.
- Allow sending credential offer email for label credentials with `VALID` status.
